...
export function NoteSearchPreview({ note, query }: { note: Note, query: string }) {
  ...
  const title = useMemo(() => {
    ...
    if (queryIndex === -1) {
      return {
        beforeText: note.title.length > 30 ? note.title.slice(0, 30) + "..." : note.title,
        query: '',
        afterText: '',
      };
    }
    ...
    return {
      ...
      afterText: note.title.slice(queryIndex + query.length, end > note.title.length ? note.title.length : end),
    };
  }, [note.title, query]);
  ...
  const content = useMemo(() => {
    ...
    if (queryIndex === -1) {
      return {
        beforeText: note.content.slice(0, 30) + (note.content.length > 30 ? "..." : ""),
        query: '',
        afterText: '',
      };
    }
    ...
    const beforeText = note.content.slice(start < 0 ? 0 : start, queryIndex).replace(/\n/g, ' ');
    const queryText = note.content.slice(queryIndex, queryIndex + queryLength).replace(/\n/g, ' ');
    const afterText = note.content.slice(queryIndex + queryLength, end > note.content.length ? note.content.length : end).replace(/\n/g, ' ') + (end > note.content.length ? '' : '...');
    ...
    if (totalLength > maxLength) {
      ...
      if (afterText.length > excessLength) {
        return {
          beforeText,
          query: queryText,
          afterText: afterText.slice(0, afterText.length - excessLength) + '...',
        };
      } else {
        return {
          beforeText: beforeText.slice(0, beforeText.length - (excessLength - afterText.length)) + '...',
          query: queryText,
          afterText,
        };
      }
  ...
  return (
    <Link href={`/note/${note._id}`} asChild>
      <TouchableOpacity className="w-full  px-8 flex flex-col justify-center text-white bg-[#303030] rounded-md"
      ...
      >
        <View className="w-full flex flex-row justify-between">
          <View>
            <Text className="text-lg font-bold">{title.beforeText}<Text 
            // highlight the query
            style={{ color: "black", backgroundColor: "yellow", borderRadius: 5
            }}
            >{title.query}</Text>{title.afterText}</Text>
          </View>
        ...
        <View className="flex gap-2 flex-row">
          <Text>{date}</Text>
          <Text>{content.beforeText}<Text 
          // highlight the query
          style={{ color: "black", backgroundColor: "yellow", borderRadius: 5
          }}
          >{content.query}</Text>{content.afterText.trimEnd()}</Text>
        </View>
      </TouchableOpacity>
    </Link>
  );
